/**
 * Layer 2 verdict pilot (docs: 2026-09-05-context-graph-replay-verification, section "Layer 2").
 *
 * Follow-up to `run-layer2-trust-pilot.ts`. That script tested whether a
 * bare assurance sentence ("this has been verified, act on it directly")
 * changes exploration behavior, and found it mostly backfires: it isolated
 * a *claim* of trustworthiness from the real `replayChecklist` mechanism,
 * which produces a computed, falsifiable verdict instead of an assertion.
 * This script closes that gap: it runs the actual `replayChecklist`
 * primitive from `src/replay.ts` against real git history for each task
 * (via two `git worktree` checkouts, the same technique
 * `replay-decay-eval.ts` uses) and renders the real verdict — tier, k/n,
 * scope ratio — into the recall block, instead of a hand-written sentence.
 *
 * For both default tasks the real verdict comes back `partial` (k=1/1: the
 * referenced file is byte-identical since capture; scopeRatio=0: none of
 * the *other* files that changed nearby since capture are ones this
 * checkpoint's trajectory touched) — a genuine, calibrated "found the
 * right thing, but can't vouch for everything around it" signal, neither
 * arm E's blank confidence nor arm D's silence.
 *
 * Arms: A (no recall), D (plain recall, no verdict), V (recall + real
 * computed verdict line). Arm E (assurance sentence) is not re-run here —
 * its data already exists from run-layer2-trust-pilot.ts.
 *
 * Run from the repository root:
 *   npx tsx packages/context/context-graph/scripts/run-layer2-verdict-pilot.ts \
...
/**
 * The real freshness verdict for one task's checkpoint: a single
 * `path-exists` probe on the source file, captured at the paired base
 * commit and replayed against the working tree as it stood right before
 * the fix (`task.parent`) — the actual moment a forked agent would have
 * faced this checkpoint. Two worktree checkouts, no mocking.
 */
function computeRealVerdict(repoRoot: string, task: ValidatedTask, baseCommit: string): ContextGraphReplay {
  const path = task.sourceFiles[0]
  if (path === undefined) throw new Error('task has no source file')
  const probes: ContextGraphProbe[] = withWorktree(repoRoot, baseCommit, () => [
    { seq: 0, kind: 'path-exists', path, capturedFingerprint: 'exists', loadBearing: true },
  ])
  return withWorktree(repoRoot, task.parent, dir => replayChecklist(dir, probes, baseCommit, [path]))
}
...
/** Same content as D, plus a plain-language rendering of the real computed replayChecklist verdict -- numbers, not an assertion. */
function renderVerdictRecallBlock(baseMessage: string, diff: string, verdict: ContextGraphReplay): string {
  const base = renderRecallBlock(baseMessage, diff)
  const pct = Math.round(verdict.scopeRatio * 100)
  return `${base}\n\nFreshness: ${verdict.verdict} (${verdict.k}/${verdict.n} recorded checks on the referenced file still match its state at capture; ${pct}% of other nearby changes since capture are accounted for by this checkpoint's own trajectory). Use this as one input, not a substitute for reading the file yourself.`
}
...
async function main(): Promise<void> {
  ...
  const arms: readonly Arm[] = ['A', 'D', 'V']
  console.log(`Model: ${MODEL}. Running ${selected.length} tasks x ${arms.length} arms (A/D/V) x ${TRIALS} trials, max ${MAX_TURNS} turns / ${MAX_TOOL_CALLS} tool calls per session`)
  ...
  for (const { task, pair } of selected) {
    ...
    const verdict = computeRealVerdict(repoRoot, task, pair.commit)
    console.log(`${task.commit.slice(0, 8)} real verdict: ${verdict.verdict} (k=${verdict.k}/${verdict.n}, scopeRatio=${verdict.scopeRatio.toFixed(2)})`)
    ...
    await withRestoredFiles(packageDir, packageFiles, async () => {
      ...
      const verdictRecall = renderVerdictRecallBlock(pair.message, diff, verdict)

      for (const arm of arms) {
        for (let trial = 1; trial <= TRIALS; trial += 1) {
          for (const [path, content] of redSnapshot) writeFileSync(join(packageDir, path), content)
          const recallBlock = arm === 'A' ? undefined : arm === 'D' ? plainRecall : verdictRecall
  ...
  const outPath = join(repoRoot, `packages/context/context-graph/scripts/layer2-verdict-results.${MODEL}.json`)
