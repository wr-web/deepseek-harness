...
> **不要按这份笔记写的去实现。** 下面"验证"一节点名了一个前置条件：在为门槛建捕获之前，先测门槛到底有没有用。那个实验后来跑了（`run-layer2-gate-pilot.ts`，结果记在[重放验证那份笔记](2026-09-05-context-graph-replay-verification.zh.md)里），而门槛输了——在真正 `dead` 的 checkpoint 上，压制召回是 0/30，照样注入是 4/30，而且贡献了全部四次成功的那个 checkpoint，是一个只改文档、改的还是一个后来被删掉的文件、跟任务毫无语义关系的 diff。
>
> 原因现在已经在全部 368 个经过 oracle 验证的会话上测出来了：被召回的块起作用的方式是充当一个*范例*，把模型"愿意动手写修复"的概率抬上去（3% → 约 22%；而动手之后 98% 都能通过），而不是作为关于当前代码的信息。所以新鲜度门槛优化的根本就是错的属性——一个 checkpoint 引用的代码还在不在，并不能预测把它召回来有没有用。
>
> 这份笔记保留下来，是因为它的地基本身仍然成立：捕获的机制、"工具名 → 路径参数名"的映射表、纯函数约束、以及对持久化接缝的分析，全都还是准确的，将来任何真正需要知道"某一轮碰过什么"的功能都会用到。被撤回的是动机——新鲜度门槛——而不是这些工程分析。
